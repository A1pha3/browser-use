...
<div className="mb-8">
  <span className="text-sm px-3 py-1 bg-purple-100 text-purple-800 rounded-full mr-2">⭐⭐⭐⭐ 专家级</span>
...
| 目标层级 | 描述 | 检验标准 |
|---------|------|---------|
| **记忆** | 说出可用的钩子类型和调用时机 | 能列举 2 种以上钩子 |
| **理解** | 解释钩子与 Agent 对象的关系 | 能说明钩子的执行流程 |
| **应用** | 使用钩子实现自定义逻辑和集成 | 能完成复杂的钩子应用 |

## 核心概念

Browser-Use 提供生命周期钩子，允许你在代理执行的特定点执行自定义代码。
...
| 场景 | 钩子类型 | 说明 |
|-----|---------|------|
| 实时监控 | `on_step_start` | 每步开始时记录状态 |
| 数据提取 | `on_step_end` | 每步结束时提取数据 |
| 条件暂停 | 两者 | 根据条件暂停执行 |
| 自定义验证 | `on_step_end` | 验证步骤结果 |
| 外部集成 | 两者 | 与外部系统同步 |

## 使用技巧

<AccordionGroup>
  <Accordion title="避免阻塞操作">
    由于钩子与代理在同一个执行线程中运行，请保持高效并避免阻塞操作。
  </Accordion>
  <Accordion title="改用自定义工具">
    钩子是相当高级的功能，大多数功能可以使用[自定义工具](/customize/tools/basics)实现
  </Accordion>
  <Accordion title="增加 step_timeout">
    如果你的钩子正在做需要很长时间的事情，你可以增加 `Agent(...)` 构造函数中的 `step_timeout` 参数。
  </Accordion>
</AccordionGroup>
